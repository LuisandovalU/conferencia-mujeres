...
/**
 * SpeakersSection component that highlights the event's speakers and worship band.
 * Uses a responsive grid to display speaker cards with specialty badges.
 * 
 * @returns {JSX.Element} The rendered speakers section.
 */
export function SpeakersSection() {
    return (
        <section id="speakers" className="py-24 bg-beige-50 relative">
            <div className="max-w-6xl mx-auto px-6">
                {/* Section Header: Left-aligned title with right-aligned metadata */}
                ...
                {/* Speakers Grid: Dynamically rendered from the speakers array */}
                <div className="grid md:grid-cols-3 gap-8">
                    {speakers.map((speaker, index) => (
                        <div
                            key={speaker.name}
                            className={`bg-white rounded-2xl p-2 border border-beige-200 group hover:-translate-y-2 transition-transform duration-300 relative ${speaker.isKeynote ? "shadow-xl z-10 md:-mt-8" : "shadow-sm"
                                }`}
                        >
                            {/* Speaker Image Container: Keynotes have different background colors */}
                            <div
                            ...
                            >
                                ...
                                />
                                {/* Overlay badge for Keynote speakers */}
                                {speaker.isKeynote && (
