            ...
            <div className="max-w-6xl mx-auto px-6 relative z-10 flex flex-col md:flex-row items-center gap-16">
                ...
                <div className="md:w-1/2 space-y-6 text-center md:text-left animate-fade-right">
                    <div className="inline-block bg-coffee-light text-white text-xs font-bold uppercase tracking-widest px-3 py-1 rounded mb-2">
                        {showForm ? 'Paso Final' : 'Última oportunidad'}
                    </div>
                    <h2 className="font-serif text-6xl text-coffee-dark tracking-tighter">
                        {showForm ? 'Completa tus datos' : 'Asegura tu lugar'}
                    </h2>
                    <p className="text-lg text-coffee-dark/70 font-light max-w-md mx-auto md:mx-0">
                        {showForm
                            ? 'Estás a solo un paso de vivir una experiencia que transformará tu vida.'
                            : 'No dejes pasar esta oportunidad. Cupo limitado para mantener la intimidad del evento.'}
                    </p>
                    {showForm && (
                        <button
                            onClick={() => setShowForm(false)}
                            className="flex items-center gap-2 text-coffee-medium hover:text-coffee-dark transition-colors font-bold text-xs uppercase tracking-widest"
                        >
                            <ArrowLeft className="w-4 h-4" /> Volver a ver detalles
                        </button>
                    )}
                </div>

                {/* Contenedor del Diseño de Ticket / Formulario */}
                <div className="md:w-1/2 w-full animate-fade-left">
                    <div className="relative max-w-sm mx-auto drop-shadow-2xl">
                        {!showForm ? (
                            /* Vista de Ticket */
                            <div className="bg-white rounded-3xl overflow-hidden border border-beige-300 hover:scale-[1.02] transition-transform duration-300">
                                {/* Cabecera del Ticket */}
                                ...
                                <div className="p-8 space-y-6 relative">
                                    ...
                                    <div className="pt-2">
                                        ...
                                        <button
                                            ...
                                            className="w-full bg-coffee-dark text-beige-100 py-4 rounded-xl font-bold tracking-wide hover:bg-coffee-medium transition-colors flex justify-center items-center gap-2 shadow-lg shadow-coffee-dark/10"
                                        >
